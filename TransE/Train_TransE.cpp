...
void prepare() {
  /*
  FILE *f1 = fopen("../data/entity2id.txt", "r"); // our data is constructed as id \t str
  FILE *f2 = fopen("../data/relation2id.txt", "r"); // our data is constructed as id \t str

  int x;

  //TODO: change read function
...
/*
  ...
  for(auto it = left_entity.begin(); it != left_entity.end(); ++it) {
    ...
    left_num[it->first] = sum2 / sum1;
  }

  for(auto it = right_entity.begin(); it != right_entity.end(); ++it) {
    double sum1 = 0, sum2 = 0;
    for (auto itt = it->second.begin(); itt != it->second.end(); ++itt) {
      sum1++;
      sum2 += itt->second;
    }
    right_num[it->first] = sum2 / sum1;
  }
/*
